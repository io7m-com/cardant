...
public final class CSAttributeNameTest
{
  ...
  public Stream<DynamicTest> testInvalids()
  {
    return Stream.of("", "A", String.format("%0257x", Integer.valueOf(123)))
      .map(CSAttributeNameTest::invalidTestOf);
  }
  ...
  public Stream<DynamicTest> testValids()
  {
    return Stream.of(
      ...
      .map(CSAttributeNameTest::validTestOf);
  }
    ...
    @StringLength(min = 1, max = 16) final String textB)
  {
    Assertions.assertEquals(
      ...
      new CSAttributeName(textA).compareTo(new CSAttributeName(textB))
    );
  }
}
