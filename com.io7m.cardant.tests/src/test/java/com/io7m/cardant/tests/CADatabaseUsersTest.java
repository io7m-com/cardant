...
public final class CADatabaseUsersTest
{
  ...
  /**
   * Creating the initial user works.
   *
   * @param transaction The transaction
   *
   * @throws Exception On errors
  ...
  @Test
  public void testUserCreateInitial0(
    final CADatabaseTransactionType transaction)
    throws Exception
  {
    final var q =
    ...
    q.userInitialSet(user.userId());
    assertEquals(user, q.userGet(user.userId()).orElseThrow());
    assertEquals(user.userId(), q.userInitial().orElseThrow());

    q.userInitialUnset(user.userId());
    assertEquals(user, q.userGet(user.userId()).orElseThrow());
    assertEquals(Optional.empty(), q.userInitial());
  }
