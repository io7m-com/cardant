...
import static com.io7m.cardant.server.inventory.v1.internal.CA1Errors.errorResponseOf;
...
public final class CA1HandlerFileUpload
  extends CAHTTPHandlerFunctional
{
    ...
    final CARequestLimits limits)
  {
    ...
    try (var resources = CA1Resources.createResources()) {
      final var input =
        resources.add(limits.boundedMaximumInputForFileUpload(request));
      final var digest =
        vHashAlgorithm.get();
      ...
      try (var digestStream = new DigestInputStream(input, digest)) {
        ...
        final var connection =
          resources.add(database.openConnection(CADatabaseRole.CARDANT));
