...
import com.io7m.cardant.server.service.configuration.v1.CAC1Configuration;
...
final class CAServerConfigurationParser
  implements CAServerConfigurationParserType
{
  ...
  {
    return "[CAServerConfigurationParser 0x%x]"
      .formatted(Integer.valueOf(this.hashCode()));
  }

  @Override
  public CAServerConfigurationFile execute()
    throws ParsingException
  {
    try {
      return BlackthorneJXE.parse(
        ...
        this.stream,
        Map.ofEntries(
          entry(qName("Configuration"), CAC1Configuration::new)
        ),
        CAServerConfigurationSchemas.schemas(),
        this.context
      );
      ...
      final var statuses =
        e.errors()
          .stream()
          .map(CAServerConfigurationParser::mapParseError)
          .toList();
