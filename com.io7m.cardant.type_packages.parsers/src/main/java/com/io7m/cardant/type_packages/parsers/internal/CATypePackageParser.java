...
public final class CATypePackageParser implements CATypePackageParserType
{
  ...
  @Override
  public CATypePackageDeclaration execute()
    throws ParsingException
  {
    try {
      return BlackthorneJXE.parse(
        ...
        this.stream,
        Map.ofEntries(
          entry(qName("Package"), CATP1Package::new)
        ),
        CATypePackageSchemas.schemas(),
        this.lexical
      );
      ...
      final var statuses =
        e.errors()
          .stream()
          .map(CATypePackageParser::mapParseError)
          .toList();
