...
import com.io7m.cardant.protocol.inventory.cb.CAI1ComparisonSet;
  ...
  CAI1ComparisonSet<W>>
{
  ...
  public CAComparisonSetType<V> convertFromWire(
    final CAI1ComparisonSet<W> message)
    throws CAProtocolException
  {
    if (message instanceof CAI1ComparisonSet.Anything<W>) {
      return new CAComparisonSetType.Anything<>();
    }

    if (message instanceof final CAI1ComparisonSet.IsEqualTo<W> e) {
      return new CAComparisonSetType.IsEqualTo<>(
        CBSets.toSet(e.fieldValue(), this::convertFromWireQuiet)
      );
    }

    if (message instanceof final CAI1ComparisonSet.IsNotEqualTo<W> e) {
      return new CAComparisonSetType.IsNotEqualTo<>(
        CBSets.toSet(e.fieldValue(), this::convertFromWireQuiet)
      );
    }

    if (message instanceof final CAI1ComparisonSet.IsSubsetOf<W> e) {
      return new CAComparisonSetType.IsSubsetOf<>(
        CBSets.toSet(e.fieldValue(), this::convertFromWireQuiet)
      );
    }

    if (message instanceof final CAI1ComparisonSet.IsSupersetOf<W> e) {
      return new CAComparisonSetType.IsSupersetOf<>(
        CBSets.toSet(e.fieldValue(), this::convertFromWireQuiet)
      );
    }

    if (message instanceof final CAI1ComparisonSet.IsOverlapping<W> e) {
