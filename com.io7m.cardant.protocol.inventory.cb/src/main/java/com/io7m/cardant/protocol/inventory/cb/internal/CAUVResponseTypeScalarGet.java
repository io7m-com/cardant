...
public enum CAUVResponseTypeScalarGet
  implements CAProtocolMessageValidatorType<
  CAIResponseTypeScalarGet, CAI1ResponseTypeScalarGet>
{
  /**
   * A validator.
   */

  RESPONSE_TYPE_SCALAR_GET;

  @Override
  public CAI1ResponseTypeScalarGet convertToWire(
    final CAIResponseTypeScalarGet c)
  {
    return new CAI1ResponseTypeScalarGet(
      new CBUUID(c.requestId()), TYPE_SCALAR.convertToWire(c.type())
    );
  }

  @Override
  public CAIResponseTypeScalarGet convertFromWire(
    final CAI1ResponseTypeScalarGet m)
  {
    return new CAIResponseTypeScalarGet(
      m.fieldRequestId().value(),
      TYPE_SCALAR.convertFromWire(m.fieldType())
    );
  }
}
